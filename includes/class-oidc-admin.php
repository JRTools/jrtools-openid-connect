...
class OIDC_Admin {
	...
	public function register_settings() {
		$options = array(
			...
			'oidc_pkce_supported'         => array( $this->sanitize, 'sanitize_checkbox' ),
			'oidc_client_id'              => 'sanitize_text_field',
			'oidc_client_secret'          => array( $this->sanitize, 'sanitize_secret' ),
			'oidc_scopes'                 => 'sanitize_text_field',
			'oidc_token_auth_method'      => 'sanitize_text_field',
			'oidc_debug_mode'             => array( $this->sanitize, 'sanitize_checkbox' ),
			'oidc_create_user'            => array( $this->sanitize, 'sanitize_checkbox' ),
			'oidc_default_role'           => 'sanitize_text_field',
			'oidc_enable_refresh'         => array( $this->sanitize, 'sanitize_checkbox' ),
			'oidc_active_claim'           => 'sanitize_text_field',
			'oidc_sync_avatar'            => array( $this->sanitize, 'sanitize_checkbox' ),
			'oidc_hide_wp_login'          => array( $this->sanitize, 'sanitize_checkbox' ),
			'oidc_auto_login'             => array( $this->sanitize, 'sanitize_checkbox' ),
			'oidc_button_icon_url'        => 'esc_url_raw',
			'oidc_token_encryption'       => array( $this->sanitize, 'sanitize_checkbox' ),
			'oidc_lock_email'             => array( $this->sanitize, 'sanitize_checkbox' ),
			'oidc_lock_password'          => array( $this->sanitize, 'sanitize_checkbox' ),
			'oidc_session_management'     => array( $this->sanitize, 'sanitize_checkbox' ),
			'oidc_remember_me'            => 'sanitize_text_field',
			'oidc_role_claim'             => 'sanitize_text_field',
			'oidc_role_mapping'           => array( $this->sanitize, 'sanitize_role_mapping' ),
		);
	...
	private function register_provider_section() {
		...
		$this->add_field( 'oidc_discovery_url', __( 'Discovery URL', 'oidc-client' ), array( $this->fields, 'field_discovery_url' ), 'oidc_section_provider' );
		$this->add_field(
			'oidc_provider_name',
		...
		$this->add_field( 'oidc_authorization_endpoint', __( 'Authorization Endpoint', 'oidc-client' ), array( $this->fields, 'field_url' ), 'oidc_section_provider', array( 'option' => 'oidc_authorization_endpoint' ) );
		$this->add_field( 'oidc_token_endpoint', __( 'Token Endpoint', 'oidc-client' ), array( $this->fields, 'field_url' ), 'oidc_section_provider', array( 'option' => 'oidc_token_endpoint' ) );
		$this->add_field( 'oidc_userinfo_endpoint', __( 'Userinfo Endpoint', 'oidc-client' ), array( $this->fields, 'field_url' ), 'oidc_section_provider', array( 'option' => 'oidc_userinfo_endpoint' ) );
		$this->add_field( 'oidc_jwks_uri', __( 'JWKS URI', 'oidc-client' ), array( $this->fields, 'field_url' ), 'oidc_section_provider', array( 'option' => 'oidc_jwks_uri' ) );
		$this->add_field(
			'oidc_pkce_supported',
	...
	private function register_client_section() {
		...
		$this->add_field( 'oidc_client_id', __( 'Client ID', 'oidc-client' ), array( $this->fields, 'field_text' ), 'oidc_section_client', array( 'option' => 'oidc_client_id' ) );
		$this->add_field( 'oidc_client_secret', __( 'Client Secret', 'oidc-client' ), array( $this->fields, 'field_password' ), 'oidc_section_client', array( 'option' => 'oidc_client_secret' ) );
		$this->add_field(
			'oidc_scopes',
		...
		$this->add_field( 'oidc_redirect_uri', __( 'Redirect URI', 'oidc-client' ), array( $this->fields, 'field_redirect_uri' ), 'oidc_section_client' );
		$this->add_field( 'oidc_token_auth_method', __( 'Token-Endpoint Authentifizierung', 'oidc-client' ), array( $this->fields, 'field_token_auth_method' ), 'oidc_section_client' );
	}
	...
	private function register_users_advanced_roles_sections() {
		...
		$this->add_field( 'oidc_default_role', __( 'Standard-Rolle für neue Benutzer', 'oidc-client' ), array( $this->fields, 'field_roles_dropdown' ), 'oidc_section_users' );
		$this->add_field(
			'oidc_debug_mode',
		...
		$this->add_field( 'oidc_end_session_endpoint', __( 'End-Session Endpoint', 'oidc-client' ), array( $this->fields, 'field_url' ), 'oidc_section_advanced', array( 'option' => 'oidc_end_session_endpoint' ) );
		...
		$this->add_field( 'oidc_active_claim', __( 'Active-Claim', 'oidc-client' ), array( $this->fields, 'field_text' ), 'oidc_section_advanced', array(
		...
		$this->add_field( 'oidc_button_icon_url', __( 'Button-Icon URL', 'oidc-client' ), array( $this->fields, 'field_url' ), 'oidc_section_advanced', array( 'option' => 'oidc_button_icon_url' ) );
		$this->add_field( 'oidc_remember_me', __( 'Angemeldet bleiben', 'oidc-client' ), array( $this->fields, 'field_remember_me' ), 'oidc_section_advanced' );
		...
		$this->add_field( 'oidc_role_mapping', __( 'Rollen-Mapping', 'oidc-client' ), array( $this->fields, 'field_role_mapping' ), 'oidc_section_roles' );
	}
